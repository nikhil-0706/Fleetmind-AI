...
def calculate_capacity_utilization_score(truck, load):
    ...
    if remaining <= 0:
        return 0
    score = (load.load_weight / remaining) * 100
    return clamp(score)

def calculate_driving_hours_feasibility_score(truck, distance_data):
    total_dist = distance_data.distance_to_pickup + distance_data.distance_pickup_to_drop
    required_time = total_dist / TRUCK_SPEED_KMPH
    if truck.remaining_drive_hours <= 0:
        return 0
    score = ((truck.remaining_drive_hours - required_time) / truck.remaining_drive_hours) * 100
    return clamp(score)
