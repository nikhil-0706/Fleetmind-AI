...
from models import ScheduledTruck, EvaluationRequest  # <-- changed from .models
...
def predict_dock_occupancy(eta: datetime, scheduled_trucks: List[ScheduledTruck], total_docks: int):
    ...
    for start, end in intervals:
        if start <= eta < end:
            occupied += 1
    free_docks = max(0, total_docks - occupied)
    wait = 0
    if free_docks == 0:
        ...
        if future_ends:
            ...
            wait = 30
    return occupied, free_docks, wait
...
def calculate_utility_score(request: EvaluationRequest, warehouse):
    ...
    utility = (0.25 * dock_score + 0.30 * wait_score + 0.15 * util_score + 0.10 * compat_score + 0.20 * deadline_score)
