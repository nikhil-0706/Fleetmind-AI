  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4">
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold text-primary">FleetMind</h1>
        <p className="text-text-light mt-2">Logistics Intelligence Platform</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-4xl w-full">
        {roles.map((role) => (
          <button
            ...
            className={`flex flex-col items-center p-6 rounded-2xl border-2 ${role.color} hover:shadow-lg transition-all transform hover:-translate-y-1`}
          >
            <role.icon size={40} className={role.textColor} />
            <span className={`mt-3 font-semibold ${role.textColor}`}>{role.name}</span>
          </button>
        ))}
      </div>
    </div>
