...
import { Grid, Truck, Package, Warehouse, Link } from 'lucide-react'
...
export default function SystemState() {
  ...
  const fetchState = async () => {
    ...
    try {
      ...
      setState({ trucks: mockTrucks, loads: mockLoads, warehouses: mockWarehouses, pairs: mockPairs })
          ...
          <tbody>
            {state.loads.map(l => (
              <tr key={l.load_id} className="border-b border-border">
                ...
                <td className="p-3">{l.pickup_node}</td>
                <td className="p-3">{l.drop_node}</td>
