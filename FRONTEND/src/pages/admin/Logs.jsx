...
export default function Logs() {
  ...
  const handleExport = async () => {
    try {
      ...
      // fallback: create CSV from current logs
      const csv = ['id,timestamp,event,entity,details', ...logs.map(l => `${l.id},${l.timestamp},${l.event},${l.entity},"${l.details}"`)].join('\n')
      ...
      a.download = 'logs.csv'
      a.click()
      toast.success('Exported mock logs')
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-5 overflow-x-auto">
        <div className="text-sm text-text-light mb-2">{filteredLogs.length} events</div>
          ...
          <tbody>
            {filteredLogs.map((log) => (
              <tr key={log.id} className="border-b border-border">
                ...
                <td className="p-3"><span className="badge-info text-xs">{log.event}</span></td>
                <td className="p-3 font-mono text-xs">{log.entity}</td>
                <td className="p-3 text-text-light text-xs">{log.details}</td>
              </tr>
            ))}
          </tbody>
        </table>
