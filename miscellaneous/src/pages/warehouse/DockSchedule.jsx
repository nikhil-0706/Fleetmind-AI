...
export default function DockSchedule() {
  ...
  const totalDocksValue = 4 // static fallback, since we don't have a getWarehouse endpoint
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-5 overflow-x-auto">
        ...
        {schedule.length === 0 ? (
          <p className="text-text-light text-center py-8">No trucks scheduled today.</p>
        ) : (
          <div className="min-w-max">
            ...
            {/* Dock rows – round robin assignment */}
            {Array.from({ length: totalDocksValue }, (_, dockIdx) => {
              const dockTrucks = schedule.filter((_, idx) => idx % totalDocksValue === dockIdx)
              return (
        ...
        <p className="text-xs text-text-light mt-4">Each block shows truck ID and unloading duration.</p>
      </div>
    </div>
  )
}